...
bool _stdcall Connect(int pID, unsigned long long bufferID)
{
	...
	{
		VirtualQueryEx(m_hProcess, address, &MemInfo, sizeof(MemInfo));
		{
...
void _stdcall ReadMessage(char receiveBuffer[], unsigned long long receiveID, bool keepMessage)
{
	if (m_bufferAddress != 0)
	{
		...
		if (iD == receiveIDCode || receiveIDCode == 0)//new message
		{
...
int _stdcall GetAddress()
{
	return (int)m_bufferAddress;
}
int _stdcall GetPageNum()
{
	return m_nPage;
}
